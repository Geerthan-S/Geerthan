...
function ClientsPanel({ data, act, saving }: { data: BusinessSnapshot; act: (action:string,payload:Record<string,unknown>)=>Promise<void>; saving:boolean }) {
  ...
  async function note(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!selected)return;const form=new FormData(event.currentTarget);await act("add_client_note",{client_id:selected.id,note:form.get("note")});event.currentTarget.reset();}
  async function contact(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!selected)return;const form=new FormData(event.currentTarget);await act("add_client_contact",{client_id:selected.id,name:form.get("name"),role:form.get("role")??"",email:form.get("email")??"",phone:"",is_primary:false});event.currentTarget.reset();}
  return <><div className="business-split"><GlassPanel className="work-hub-panel"><div className="module-heading"><div><span className="eyebrow">Directory</span><h2>Client relationships</h2></div><UsersRound size={18}/></div><div className="client-card-grid">{data.clients.map((client)=><button className={`client-mini-card ${selected?.id===client.id?"is-selected":""}`} onClick={()=>setSelectedId(client.id)} key={client.id}><div><span className={`record-dot health-${client.health}`}/><strong>{client.company || client.name}</strong></div><p>{client.name} · {client.status}</p><small>{client.nextAction || "No next action"}</small><span>{data.contacts.filter((item)=>item.clientId===client.id).length} contacts · {data.notes.filter((item)=>item.clientId===client.id).length} notes</span></button>)}</div></GlassPanel><GlassPanel className="compact-create-panel"><span className="eyebrow">New relationship</span><h2>Add a client</h2><form onSubmit={submit}><input className="glass-input" name="name" required placeholder="Contact name"/><input className="glass-input" name="company" placeholder="Company"/><input className="glass-input" name="email" type="email" placeholder="Email"/><Button disabled={saving}><Plus size={14}/>Add client</Button></form></GlassPanel></div>{selected?<GlassPanel className="client-detail"><div className="module-heading"><div><span className="eyebrow">Client detail</span><h2>{selected.company||selected.name}</h2></div><span className={`connector-status ${selected.health==="on_track"?"healthy":""}`}>{selected.health.replaceAll("_"," ")}</span></div><div className="client-detail-grid"><section><h3>Relationship</h3><p>{selected.name} · {selected.email||"No email"}</p><small>Next: {selected.nextAction||"No action defined"} · {dateLabel(selected.nextActionAt)}</small><h3>Contacts</h3>{data.contacts.filter((item)=>item.clientId===selected.id).map((item)=><div className="client-detail-row" key={item.id}><strong>{item.name}</strong><span>{item.role} · {item.email}</span></div>)}</section><section><h3>Delivery and money</h3>{data.deliverables.filter((item)=>item.clientId===selected.id).map((item)=><div className="client-detail-row" key={item.id}><strong>{item.title}</strong><span>{item.progress}% · due {dateLabel(item.dueAt)}</span></div>)}{data.invoices.filter((item)=>item.clientId===selected.id).map((item)=><div className="client-detail-row" key={item.id}><strong>{item.invoiceNumber}</strong><span>{item.status} · {money(item.total,item.currency)}</span></div>)}</section><section><h3>Notes</h3>{data.notes.filter((item)=>item.clientId===selected.id).map((item)=><div className="client-detail-row" key={item.id}><span>{item.note}</span></div>)}<form onSubmit={note}><textarea className="glass-input" name="note" required placeholder="Add client context"/><Button disabled={saving}>Add note</Button></form></section><section><h3>Add contact</h3><form onSubmit={contact}><input className="glass-input" name="name" required placeholder="Name"/><input className="glass-input" name="role" placeholder="Role"/><input className="glass-input" name="email" type="email" placeholder="Email"/><Button disabled={saving}>Add contact</Button></form></section></div></GlassPanel>:null}</>;
}
